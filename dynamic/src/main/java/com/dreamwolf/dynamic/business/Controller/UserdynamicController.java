...
public class UserdynamicController {
    ...
    public ResponseData<List<Cards>> dynamic_new(){
        Integer id=1;
        ...
        for (int i=0;i<userdynamic.size();i++){
            ...
            cards.add(new Cards(desc));
        }
        return new  ResponseData<List<Cards>>(0,"",1,cards);
    }
    ...
    public ResponseData<List<Cards>> dynamic_history(Integer offset_dynamic_id){
        Integer id=1;
        List<Relations> listMap=memberService.intuid(id).getData();
    ...
    public  ResponseData<List<DynamicNumObject>> dynamic_num(Integer [] uids){
        ...
        return new ResponseData<List<DynamicNumObject>>(code,message,1,dynamicNumObjectList);
    }
    ...
    public ResponseData<Desc> dynamic_detail(Integer dynamic_id){
        Integer id=1;
        QueryWrapper<Userdynamic> wrapper = new QueryWrapper<>();//条件构造器
        ...
        Desc desc=new Desc(
                ...
                userdynamic.getUdID(),
                new UserProfile(member,vipStatus,level_info));
        return new ResponseData<Desc>(0,"",1,desc);
    }
